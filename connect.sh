#!/bin/bash
# This script connects to a reflector only if there has been no activity
# on the local repeater for the last 5 minutes.
#
# (c) 2024 Fred W6BSD.
#
# Usage:
# $ connect.sh ref014_c	   # This command will link to the reflector REF014_C.
# $ connect.sh         	   # This command will unlink from any reflector.
...
if [ "$inactivity_duration" -ge "$INACTIVITY_THRESHOLD" ]; then
  ...
  /usr/local/sbin/pistar-link $REFLECTOR
